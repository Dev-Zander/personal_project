...
class UpcomingTrips extends Component {
    ...
    handleDeleteClick(id) {
        axios.delete(`/api/trip/${id}`).then(res => {
            ...
            this.setState({ trips: res.data, myTrips:res.data })
        }).then(
            res => {
                swal({
                    text: "Trip Deleted!",
                    ...
                    <section id="trip_list">
                        <ul className="ul">
                            ...
                            <div className='Results1'>
                                <div style={{marginTop:47, marginLeft:-287}} className="heading"> Trip Name:  </div>
                                <li  style={{marginTop:50, marginLeft:5}}className="li">{trip.trip_name}</li> </div>
